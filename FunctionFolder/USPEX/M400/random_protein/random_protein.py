...
def generate_structure(passed_angles, pickup):
    ...
    os.system('minimize ' + xyz_file  +' '+ str(precision_optimize) + ' > ' + tmp_file)
    ...
    os.system('xyzint ' + xyz2_file + ' A > /dev/null 2>&1')
    os.system('rm -f '    + pdb_file  + ' > /dev/null 2>&1')
    os.system('xyzpdb ' + xyz2_file + ' ' + key_file + ' > /dev/null 2>&1')

    #Расчет RMSD между релаксированной и реальной структурой, лежащей в Specific
    os.system("printf '3\n3\n' | gmx confrms -f1 " + pdb_file + " -f2 ../Specific/*.pdb > rmsd.log")
